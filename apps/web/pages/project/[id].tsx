...
import { getEnvVars, pushEnvVars, getMembers, inviteMember, removeMember, Environment, ProjectMember } from '@/lib/api';
        ...
        <div className="mb-6 flex gap-1 rounded-lg border border-gray-200 bg-gray-50 p-1 w-fit">
          <button
            ...
            className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${
              tab === 'variables' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            {t('editButton').split(' ').pop() === 'variables' ? 'Variables' : 'Variables'}
          </button>
          <button
        ...
        {tab === 'members' && (
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              <h3 className="mb-4 text-sm font-semibold text-gray-700">{tMembers('inviteTitle')}</h3>
              ...
              {pendingInviteLink && (
                <div className="mt-4 flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 p-3">
                  <span className="flex-1 truncate font-mono text-xs text-indigo-700">{pendingInviteLink}</span>
                  <button
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
              {membersLoading ? (
                ...
                <div className="px-6 py-12 text-center text-sm text-gray-400">{tMembers('empty')}</div>
              ) : (
                <table className="w-full text-sm">
                  <tbody className="divide-y divide-gray-50">
                    {members.map((member) => (
                      <tr key={member.id} className="group hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                            ...
                            {member.role === 'EDITOR' ? tMembers('roleEditor') : tMembers('roleViewer')}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
