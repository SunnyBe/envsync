...
export default function ProjectPage() {
  ...
  // Seed draft from current vars when the user clicks "Edit" — called imperatively,
  // not via useEffect, to avoid the setState-in-effect lint pattern.
  ...
  const pushMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`${env} variables saved`);
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
              <div className="border-b border-gray-100 px-4 py-3 flex items-center justify-between">
                <span className="text-sm font-medium text-gray-700">
                  Edit <Badge variant={ENV_BADGE[env]}>{env}</Badge> variables
                </span>
                <span className="text-xs text-gray-400">One KEY=VALUE per line. Lines starting with # are ignored.</span>
              </div>
              <textarea
                value={draftText}
          ...
          <>
            {entries.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-white py-20 text-center">
                ...
                <p className="mb-1 font-medium text-gray-600">No variables for {env}</p>
                <p className="mb-4 text-sm text-gray-400">Click &quot;Edit variables&quot; to add them.</p>
                <Button size="sm" onClick={enterEditMode}>Add variables</Button>
              </div>
            ) : (
              <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 w-1/3">Key</th>
                ...
                <div className="border-t border-gray-100 px-4 py-3 flex items-center justify-between">
                  <span className="text-xs text-gray-400">{entries.length} variable{entries.length !== 1 ? 's' : ''}</span>
