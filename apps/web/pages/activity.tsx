      ...
      <div className="mx-auto max-w-3xl px-8 py-8">
        ...
        {isLoading ? (
          ...
          <div className="space-y-8">
            {dateKeys.map((date) => (
              <div key={date}>
                <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-400">{date}</h3>
                <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-50">
                  {grouped[date].map((event) => {
                    const actionLabel = (() => {
                      try { return tActions(event.action as Parameters<typeof tActions>[0]); } catch { return event.action; }
                    })();
                    return (
                      <div key={event.id} className="flex items-start gap-4 px-5 py-4">
                        <div className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${getActionColor(event.action)}`}>
                          ...
                          {event.metadata && Object.keys(event.metadata).length > 0 && (
                            <div className="mt-1 flex flex-wrap gap-1">
                              {Object.entries(event.metadata).map(([k, v]) => (
                                <span key={k} className="inline-flex items-center rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500">
                                  {k}: {String(v)}
                                </span>
                              ))}
                            </div>
